...
/**
 * 基于 Redis Streams 的下载任务消费者。
 */
@Component
@ConditionalOnProperty(name = "mq.engine", havingValue = "redis")
public class DownloadTaskRedisConsumer {

    private static final Logger logger = LoggerFactory.getLogger(DownloadTaskRedisConsumer.class);

    private static final String STREAM_KEY = "stream:download-tasks";
    private static final String GROUP = "download-consumer-group";
    private static final String CONSUMER_NAME_PREFIX = "download-";
    ...
    public void init() {
        if (redisTemplate == null) {
            logger.warn("⚠️ RedisTemplate 未注入，DownloadTaskRedisConsumer 将不会工作");
            return;
        }
        ...
        try {
            ...
            for (MapRecord<String, Object, Object> record : records) {
                ...
                try {
                    ...
                    DownloadTaskMessage message = objectMapper.readValue(payload.toString(), DownloadTaskMessage.class);
                    downloadTaskConsumer.onMessage(message);
                    redisTemplate.opsForStream().acknowledge(STREAM_KEY, GROUP, record.getId());
                } catch (Exception e) {
                    logger.error("❌ [Redis] 处理 DownloadTaskMessage 失败 - id: {}", record.getId(), e);
                }
            }
        } catch (Exception e) {
            logger.error("💥 [Redis] 读取 DownloadTask 流异常", e);
        }
    }
