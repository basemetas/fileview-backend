 ...
 * 
 * 支持PPT格式（共13种源格式）：
 * - Microsoft PowerPoint: ppt, pptx, pps, ppsx, pot, potx, pptm, potm, ppam, ppsm
 * - 开放文档: odp, otp, fodp
 * - 其他格式: keynote
 * 
 * 特性：
 * - 智能引擎选择和故障转移
 * - 支持多种输出格式（pdf, png, jpg, odp等）
 * - 临时文件HTTP访问服务
 * - 完善的错误处理和日志记录
 * - PPT专用优化（动画、过渡效果支持）
 * 
...
@ConvertStrategy(
    ...
    description = "PPT演示文稿转换（LibreOffice）",
    priority = 100
)
public class PptConvertStrategy implements FileConvertStrategy {
    private static final Logger logger = LoggerFactory.getLogger(PptConvertStrategy.class);
    ...
    public void init() {
        ...
        // 从格式注册中心获取格式信息
        Set<String> sourceFormats = fileTypeMapper.getSupportedSourceFormats(FileCategory.PRESENTATION);
        Set<String> targetFormats = fileTypeMapper.getSupportedTargetFormats(FileCategory.PRESENTATION);
        
        logger.info("PPT转换策略初始化完成 - 类型: PRESENTATION, 支持源格式({}种): {}", 
    ...
    public boolean convert(String sourceFilePath, String targetFilePath) {
        logger.warn("使用已废弃的PPT转换方法，请使用带格式参数的转换方法");
        return false;
    }

    /**
     * 转换PPT文件（增强版，支持多引擎智能选择）
    ...
    public boolean convert(String sourceFilePath, String targetPath, String targetFileName, String targetFormat) {
        logger.info("开始PPT多引擎转换 - 源文件: {}, 目标路径: {}, 目标文件名: {}, 目标格式: {}", 
        ...
        try {
            ...
            if (sourceFormat == null || sourceFormat.isEmpty()) {
                logger.error("无法检测PPT源文件格式: {}", correctedSourceFilePath);
                return false;
            }
            
            // 4. 使用格式注册中心验证格式支持
            if (!fileTypeMapper.isConversionSupported(FileCategory.PRESENTATION, sourceFormat, targetFormat)) {
                logger.error("不支持的PPT格式转换: {} -> {}", sourceFormat, targetFormat);
                logger.error("支持的源格式: {}", fileTypeMapper.getSupportedSourceFormats(FileCategory.PRESENTATION));
                logger.error("支持的目标格式: {}", fileTypeMapper.getSupportedTargetFormats(FileCategory.PRESENTATION));
                return false;
            }
            
            // 5. 使用引擎选择器进行智能转换
                ConvertResult result = engineSelector.convertDocument(
                ConvertEngineSelector.DocumentType.PPT, correctedSourceFilePath, targetFilePath, sourceFormat, targetFormat);
            
            if (result.isSuccess()) {
                logger.info("✅ PPT文档转换成功 - 源文件: {}, 目标文件: {}, 使用引擎: {}", 
                           correctedSourceFilePath, targetFilePath, result.getEngine() != null ? result.getEngine().getDescription() : "未知引擎");
                return true;
            } else {
                // 🔑 关键修复：检测引擎能力不支持错误并抛出异常
                String errorCode = result.getErrorCode();
                if (errorCode != null && !errorCode.isEmpty()) {
                    logger.warn("⚠️ PPT转换失败，抛出引擎不支持异常 - ErrorCode: {}, Message: {}", 
                               errorCode, result.getMessage());
                    throw new UnsupportedException(
                        errorCode, result.getMessage()
                    );
                }
                logger.error("❌ PPT文档转换失败 - 源文件: {}, 错误: {}", 
            ...
            // 🔑 关键修复：引擎不支持异常需要重新抛出
            logger.warn("⚠️ PPT转换策略 catch 到引擎不支持异常，重新抛出 - ErrorCode: {}", e.getErrorCode());
            throw e; // 重新抛出，让上层捕获
        } catch (Exception e) {
            logger.error("PPT转换异常 - 源文件: {}", sourceFilePath, e);
            return false;
        }
    }
    ...
    public boolean convertWithParams(String filePath, String targetFilePath, String targetFileName, 
        ...
        logger.info("开始PPT文档转换（带参数） - 源文件: {}, 目标路径: {}, 文件名: {}, 格式: {}", 
        ...
        try {
            ...
            if (sourceFormat == null || sourceFormat.isEmpty()) {
                logger.error("无法检测PPT源文件格式: {}", correctedSourcePath);
                return false;
            }
            ...
            ConvertResult result = engineSelector.convertDocument(
                ConvertEngineSelector.DocumentType.PPT, correctedSourcePath, fullTargetPath, 
            ...
            if (result.isSuccess()) {
                ...
                logger.error("❌ PPT文档转换失败 - 源文件: {}, 错误: {}", correctedSourcePath, result.getMessage());
                return false;
            }
            ...
            logger.error("PPT转换异常 - 源文件: {}", filePath, e);
            return false;
        }
    }
    ...
    public boolean isConversionSupported(String sourceFormat, String targetFormat) {
        return fileTypeMapper.isConversionSupported(FileCategory.PRESENTATION, sourceFormat, targetFormat);
    }
    ...
    public Set<String> getSupportedSourceFormats() {
        return fileTypeMapper.getSupportedSourceFormats(FileCategory.PRESENTATION);
    }
    ...
    public Set<String> getSupportedTargetFormats() {
        return fileTypeMapper.getSupportedTargetFormats(FileCategory.PRESENTATION);
    }
    ...
    public EngineStatus getEngineStatus() {
        return engineSelector.getEngineStatus(ConvertEngineSelector.DocumentType.PPT);
    }
}