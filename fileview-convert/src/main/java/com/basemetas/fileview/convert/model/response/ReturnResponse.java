 ...
 */
package com.basemetas.fileview.convert.model.response;
...
public class ReturnResponse<T> {
    ...
    public static ResponseEntity<Map<String, Object>> error(HttpStatus status, String errorMessage) {
        ReturnResponse<Object> response = new ReturnResponse<>(status.value(), errorMessage, null);
     ...
     * 创建失败响应（带HTTP状态码整数）
     */
    public static ResponseEntity<Map<String, Object>> error(int statusCode, String errorMessage) {
        ReturnResponse<Object> response = new ReturnResponse<>(statusCode, errorMessage, null);
        return ResponseEntity.status(statusCode).body(response.toMap());
    }
