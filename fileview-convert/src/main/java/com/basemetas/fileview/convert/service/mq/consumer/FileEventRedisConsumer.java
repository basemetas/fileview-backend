...
/**
 * 基于 Redis Streams 的 FileEvent 消费者。
 */
@Component
@ConditionalOnProperty(name = "mq.engine", havingValue = "redis")
public class FileEventRedisConsumer {

    private static final Logger logger = LoggerFactory.getLogger(FileEventRedisConsumer.class);

    private static final String STREAM_KEY = "stream:file-events";
    private static final String GROUP = "file-convert-consumer-group";
    private static final String CONSUMER_NAME_PREFIX = "convert-";
    ...
    public void init() {
        if (redisTemplate == null) {
            logger.warn("⚠️ RedisTemplate 未注入，FileEventRedisConsumer 将不会工作");
            return;
        }
    ...
    public void consume() {
        ...
        try {
            ...
            if (records != null && !records.isEmpty()) {
                logger.info("📨 [Redis] 读取到 {} 条消息", records.size());
            }
            ...
            for (MapRecord<String, Object, Object> record : records) {
                ...
                try {
                    ...
                    FileEvent event = objectMapper.readValue(payload.toString(), FileEvent.class);
                    fileEventConsumer.onMessage(event);
                    redisTemplate.opsForStream().acknowledge(STREAM_KEY, GROUP, record.getId());
                } catch (Exception e) {
                    logger.error("❌ [Redis] 处理 FileEvent 失败 - id: {}", record.getId(), e);
                    // 不ACK，留在pending，后续可考虑重试/补偿
                }
            }
        } catch (Throwable t) {
            logger.error("💥 [Redis] 读取 FileEvent 流异常，定时任务将继续", t);
        }
    }
